...
from model.models import Object, Region, ResourceType, WaterType, WaterHistory
...
async def dal_get_object_detail_by_id(obj_id: int, db: AsyncSession) -> Object:
    result = await db.execute(
        select(Object)
        .options(
            selectinload(Object.region),
            selectinload(Object.resource_type),
            selectinload(Object.water_type),
            # ГЛАВНОЕ — ВЛОЖЕННЫЙ selectinload!
            selectinload(Object.histories)
                .selectinload(WaterHistory.history),
            selectinload(Object.histories)
                .selectinload(WaterHistory.group)
        )
        .where(Object.id == obj_id)
    )
    return result.scalar_one_or_none()